...
import com.medinfi.utils.GcmRegister;
import com.medinfi.utils.Utils;
import com.mobileapptracker.MobileAppTracker;

public class SplashActivity extends Activity implements ConnectionCallbacks, OnConnectionFailedListener, LocationListener {
	...
	String trackUserId = Secure.getString(getContentResolver(), Secure.ANDROID_ID);
	...
	ApplicationSettings.putPref(AppConstants.DEVICE_ID, trackUserId);
	
	if(ApplicationSettings.getPref(AppConstants.USER_ID, "").equals(""))
		ApplicationSettings.putPref(AppConstants.USER_ID, "0");
	
	ApplicationSettings.putPref(AppConstants.OS,"Android "+myVersion +"/ "+sdkVersion );
	ApplicationSettings.putPref(AppConstants.DEVICE_TYPE,isTablet(this)?"Tablet":"Phone" );
	ApplicationSettings.putPref(AppConstants.LATITUDE_AUTO, ""); 
	ApplicationSettings.putPref(AppConstants.LONGITUDE_AUTO, "");
	ApplicationSettings.putPref(AppConstants.GPRS_STATUS, "0");
	ApplicationSettings.putPref(AppConstants.CURRENT_USER_SUB_LOCALITY_AUTO, "");
	ApplicationSettings.putPref(AppConstants.CURRENT_USER_LOCATION_AUTO, "");

	
	
	ApplicationSettings.putPref(AppConstants.LAST_TRACK_REQ_TIME, "0");
	
	// new CurrentSessionId().execute();
	easyTracker = EasyTracker.getInstance(SplashActivity.this);
	splashActivity = this;
	...
	if (Utils.isNetworkAvailable(this)) {
	   //Log.i("HERE","HERE N/w Avai");
	    if (checkPlayServices()) {
	    	//Log.i("HERE","HERE Play Sevice Avail");
	    	//Registering for push notification
	    	new GcmRegister(getApplicationContext()).GCMRegister();
	    	
	    	loadNextAct=true;
	    	
		buildGoogleApiClient();
		createLocationRequest();
		cutOffTimer=new CountDownTimer(CUTOFF_TIMER,CUTOFF_TIMER) {

            public void onTick(long millisUntilFinished) {
            	//Log.i("Narcos","Narcos time remaining: "+(millisUntilFinished/1000));
            }
	...
	if (!isEventPressed) {
	    if (isHomePressed) {
		easyTracker.send(MapBuilder.createEvent("Splash", "Home Button", "Home", null).build());
	    }
	}
    }
	...
	FlurryAgent.onStartSession(this, getResources().getString(R.string.FlurryAPIKey));
	 FlurryAgent.logEvent("Splash Activity");
...
//	    try {
//	    	
//	    		Log.i("WRes","WRes Address : "+Arrays.toString(addresses.toArray()));
//		try {
...
//			   Log.i("WRes","WRes str Result : "+strRresult);
...
//	    }
    ...
    private void setDefault() {
    	...
    	if(!isFetchComplete){
    	//Log.i("Narcos","Narcos inside Medellin :P(Default)");
	    ...
	    startActivity(new Intent(SplashActivity.this, HospitalsActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
	...
	btnCancel.setOnClickListener(new OnClickListener() {
		...
		Intent intent = new Intent(SplashActivity.this, LocationUpdateActivity.class);
	...
	//	if (cityValidationList != null && cityValidationList.size() > 1) {//Remove 
		    isLocationTrue = true;
		    String city="";
		    //String detectedCity=loc;
		    //if (validateCity(cityValidationList)) {//Upgrade this to get this value from server
